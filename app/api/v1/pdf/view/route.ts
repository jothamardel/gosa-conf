...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      record = await ConventionRegistration.findOne({ paymentReference }).populate('userId', 'name email phoneNumber');
      if (record) {
        operationType = 'convention';
        qrCodeData = record.qrCodes?.[0]?.qrCode || `GOSA2025-CONV-${record._id}`;
      }
    } catch (error) {
    ...
    if (!record) {
      try {
        record = await DinnerReservation.findOne({ paymentReference }).populate('userId', 'name email phoneNumber');
        if (record) {
          operationType = 'dinner';
          qrCodeData = record.qrCodes?.[0]?.qrCode || `GOSA2025-DINNER-${record._id}`;
        }
      } catch (error) {
    ...
    if (!record) {
      try {
        record = await Accommodation.findOne({ paymentReference }).populate('userId', 'name email phoneNumber');
        if (record) {
          operationType = 'accommodation';
          qrCodeData = record.qrCodes?.[0]?.qrCode || `GOSA2025-ACCOM-${record._id}`;
        }
      } catch (error) {
    ...
    if (!record) {
      try {
        record = await ConventionBrochure.findOne({ paymentReference }).populate('userId', 'name email phoneNumber');
        if (record) {
          operationType = 'brochure';
          qrCodeData = record.qrCodes?.[0]?.qrCode || `GOSA2025-BROCHURE-${record._id}`;
        }
      } catch (error) {
    ...
    if (!record) {
      try {
        record = await GoodwillMessage.findOne({ paymentReference }).populate('userId', 'name email phoneNumber');
        if (record) {
    ...
    if (!record) {
      try {
        record = await Donation.findOne({ paymentReference }).populate('userId', 'name email phoneNumber');
        if (record) {
    ...
    const pdfData: PDFData = {
      ...
      operationDetails: {
        ...
        amount: record.totalAmount || record.donationAmount || record.amount || 0,
